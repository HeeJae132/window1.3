document.addEventListener('DOMContentLoaded', function() {
  ...
  const musicImage = document.querySelector('.music-image'); // 음악 이미지를 담는 div
  
  const tracks = ['audio2.mp3', 'audio1.mp3', 'audio3.mp3']; // 재생할 음악 파일들
  const trackImages = ['ticktak.png', 'Lease.png', 'fridaytheme.png']; // 트랙별 이미지
  let currentTrackIndex = 0; // 현재 재생 중인 트랙 인덱스
  
  // 초기 트랙 설정
  audio.src = tracks[currentTrackIndex];
  musicImage.style.backgroundImage = `url(${trackImages[currentTrackIndex]})`; // 초기 이미지 설정
  ...
  function updateMusicImage() {
    musicImage.style.backgroundImage = `url(${trackImages[currentTrackIndex]})`;
  }
